...
Module modJobSchedule
    ...
    Function JobSchedule_FillDailySchedule(ByRef oForm As SAPbouiCOM.Form, ByRef sErrDesc As String) As Long
        ...
        Try
            ...
            For imjs As Integer = 1 To oMatrix_1.VisualRowCount
    ...
    Function JobSchedule_DeleteEmptyRows(ByRef oForm As SAPbouiCOM.Form, ByRef sErrDesc As String) As Long
        ...
        Try
            ...
            sQueryString = "DELETE FROM [@AB_JOBSCH1] WHERE DocEntry ='" & oForm.Items.Item("txtDocEntr").Specific.string & "' AND ISNULL(U_Location,'') = '';"

            sQueryString += " DELETE FROM [@AB_JOBSCH4] WHERE DocEntry ='" & oForm.Items.Item("txtDocEntr").Specific.string & "' AND  ISNULL(U_Path,'') = ''"

            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Executing Query : " & sQueryString, sFuncName)
            oRS.DoQuery(sQueryString)

            'sQueryString = "UPDATE [@AB_JOBSCH3] SET U_TaskStatus = 'Closed' WHERE DocEntry = '" & sJobSchEntry & "'"
            'sQueryString += "UPDATE [@AB_JOBSCH2] SET U_DayStatus = 'Closed' WHERE DocEntry = '" & sJobSchEntry & "'"
            'sQueryString += "UPDATE [@AB_JOBSCH] SET U_DocStatus = 'C' WHERE DocEntry = '" & sJobSchEntry & "'"
            'oRS.DoQuery(sQueryString)
        ...
        Try
            ...
            ElseIf oMatrix.RowCount > 0 Then
                ...
                For iRowCount As Integer = 1 To oMatrix.VisualRowCount
                    ...
                    Dim sLocation As String = oMatrix.Columns.Item("V_6").Cells.Item(iRowCount).Specific.string
                    Dim sService As String = oMatrix.Columns.Item("V_5").Cells.Item(iRowCount).Specific.string
    ...
    Function JobSchedule_Validation_New(ByRef oForm As SAPbouiCOM.Form, ByRef sErrDesc As String) As Long
        ...
        Try
            ...
            ElseIf oMatrix.RowCount > 1 Then
                ...
                Dim sLocation As String = oMatrix.Columns.Item("V_6").Cells.Item(oMatrix.VisualRowCount).Specific.string
                Dim sService As String = oMatrix.Columns.Item("V_5").Cells.Item(oMatrix.VisualRowCount).Specific.string
                If sLocation = "" And sService = "" Then
                    oForm.Freeze(True)
                ...
                For i As Integer = 1 To oMatrix.VisualRowCount
                    oCombo = oMatrix.Columns.Item("V_0").Cells.Item(i).Specific

                    If oMatrix.Columns.Item("V_6").Cells.Item(i).Specific.string = "" Then
                        sErrDesc = "Location Field. Cannot be Blank!. RowNo : " & i
                        oMatrix.Columns.Item("V_6").Cells.Item(i).Click(SAPbouiCOM.BoCellClickType.ct_Regular)
                        Return RTN_ERROR
                    ElseIf oMatrix.Columns.Item("V_5").Cells.Item(i).Specific.string = "" Then
                        sErrDesc = "Cleaning Type Field. Cannot be Blank!. RowNo : " & i
                        oMatrix.Columns.Item("V_5").Cells.Item(i).Click(SAPbouiCOM.BoCellClickType.ct_Regular)
                        Return RTN_ERROR
                    ElseIf oMatrix.Columns.Item("V_4").Cells.Item(i).Specific.string = "" Then
                        sErrDesc = "Start Date Field. Cannot be Blank!. RowNo : " & i
                        oMatrix.Columns.Item("V_4").Cells.Item(i).Click(SAPbouiCOM.BoCellClickType.ct_Regular)
                        Return RTN_ERROR
                    ElseIf oMatrix.Columns.Item("V_3").Cells.Item(i).Specific.string = "" Then
                        sErrDesc = "Start Time Field. Cannot be Blank!. RowNo : " & i
                        oMatrix.Columns.Item("V_3").Cells.Item(i).Click(SAPbouiCOM.BoCellClickType.ct_Regular)
                        Return RTN_ERROR
                    ElseIf oMatrix.Columns.Item("V_2").Cells.Item(i).Specific.string = "" Then
                        sErrDesc = "End Time Field. Cannot be Blank!. RowNo : " & i
                        oMatrix.Columns.Item("V_2").Cells.Item(i).Click(SAPbouiCOM.BoCellClickType.ct_Regular)
                        Return RTN_ERROR

                    ElseIf oCombo.Value = "" Then
                        sErrDesc = "Frequency Field. Cannot be Blank!. RowNo : " & i
                        oMatrix.Columns.Item("V_0").Cells.Item(i).Click(SAPbouiCOM.BoCellClickType.ct_Regular)
                        Return RTN_ERROR
                    End If
                Next
            End If
            ...
            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed with SUCCESS", sFuncName)
            JobSchedule_Validation_New = RTN_SUCCESS

        Catch ex As Exception
            sErrDesc = ex.Message().ToString()
            Call WriteToLogFile(sErrDesc, sFuncName)
            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed with ERROR", sFuncName)
            JobSchedule_Validation_New = RTN_ERROR
        End Try
    End Function
    ...
    Public Function DuplicateJobSchedule(ByRef oForm As SAPbouiCOM.Form, ByRef sErrDesc As String) As Long
        Dim sDocEntry As String = String.Empty
        Dim sQuery As String = String.Empty
        Dim oEdit As SAPbouiCOM.EditText
        ...
        Try
            sFuncName = "DuplicateJobSchedule"

            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Starting Function ", sFuncName)

            If oForm.Mode = SAPbouiCOM.BoFormMode.fm_OK_MODE Then

                oEdit = oForm.Items.Item("txtDocEntr").Specific
                sDocEntry = oEdit.Value
                sJobSchEntry = oEdit.Value

                oForm.Mode = SAPbouiCOM.BoFormMode.fm_ADD_MODE

                oForm.Items.Item("matJobSch").Enabled = True
                oForm.Items.Item("txtDocDate").Enabled = True
                oEdit = oForm.Items.Item("txtDocDate").Specific
                oEdit.String = "s"
                oForm.Items.Item("txtJobSite").Enabled = True
                oCombo = oForm.Items.Item("cmbStatus").Specific
                oCombo.Select(0, SAPbouiCOM.BoSearchKey.psk_Index)
                ...
                oForm.Items.Item("txtPreBy").Enabled = True
                oForm.Items.Item("txtPreDate").Enabled = True
                oEdit = oForm.Items.Item("txtPreDate").Specific
                oEdit.String = "s"
                oForm.Items.Item("txtApprBy").Enabled = True
                oForm.Items.Item("txtApprDat").Enabled = True
                oEdit = oForm.Items.Item("txtApprDat").Specific
                oEdit.String = "s"
                ...
                oMatrix.Clear()
                Dim i As Integer = 1
                sQuery = "SELECT (SELECT MAX(DocNum) + 1 FROM [@AB_JOBSCH]) [DocNum],A.U_DocDate,A.U_OcrCode,A.U_DocStatus,CONVERT(VARCHAR(10),A.U_StartDate,112) [U_StartDate],CONVERT(VARCHAR(10),A.U_EndDate,112) [U_EndDate], " & _
                         " A.U_PreparedByID,A.U_PreparedBy,CONVERT(VARCHAR(10),A.U_PreparedDate,112) [U_PreparedDate],A.U_ApprovedByID,A.U_ApprovedBy,CONVERT(VARCHAR(10),A.U_ApprovedDate,112) [U_ApprovedDate] , " & _
                         " B.U_Location,B.U_CleaningType,CONVERT(VARCHAR(10),B.U_StartDate,112) [LStartDate],B.U_StartTime, B.U_EndTime,B.U_Frequency " & _
                         " FROM [@AB_JOBSCH] A INNER JOIN [@AB_JOBSCH1] B ON B.DocEntry = A.DocEntry WHERE A.DocEntry = '" & sDocEntry & "' AND ISNULL(B.U_Location,'') <> ''"
                ...
                If Not (oRecordSet.BoF And oRecordSet.EoF) Then
                    ...
                    oEdit = oForm.Items.Item("txtDocNum").Specific
                    oEdit.Value = oRecordSet.Fields.Item("DocNum").Value
                    oEdit = oForm.Items.Item("txtJobSite").Specific
                    oEdit.Value = oRecordSet.Fields.Item("U_OcrCode").Value
                    oEdit = oForm.Items.Item("txtEndDate").Specific
                    oEdit.String = oRecordSet.Fields.Item("U_EndDate").Value
                    oEdit = oForm.Items.Item("txtPreBy").Specific
                    oEdit.Value = oRecordSet.Fields.Item("U_PreparedByID").Value
                    oEdit = oForm.Items.Item("txtPreByNa").Specific
                    oEdit.Value = oRecordSet.Fields.Item("U_PreparedBy").Value
                    oEdit = oForm.Items.Item("txtApprBy").Specific
                    oEdit.Value = oRecordSet.Fields.Item("U_ApprovedByID").Value
                    oEdit = oForm.Items.Item("txtAppByNa").Specific
                    oEdit.Value = oRecordSet.Fields.Item("U_ApprovedBy").Value
                    ...
                    Do Until oRecordSet.EoF
                        ...
                        oCombo = oMatrix.Columns.Item("V_0").Cells.Item(i).Specific
                        Try
                            oCombo.Select(oRecordSet.Fields.Item("U_Frequency").Value, SAPbouiCOM.BoSearchKey.psk_ByValue)
                        Catch ex As Exception
                            Call WriteToLogFile("Error while selecting combo value in matrix", sFuncName)
                            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Error while selecting combo value in matrix", sFuncName)
                        End Try
                        oMatrix.Columns.Item("V_2").Cells.Item(i).Specific.value = oRecordSet.Fields.Item("U_EndTime").Value
                        oMatrix.Columns.Item("V_3").Cells.Item(i).Specific.value = oRecordSet.Fields.Item("U_StartTime").Value
                        oMatrix.Columns.Item("V_4").Cells.Item(i).Specific.string = "s"
                        oMatrix.Columns.Item("V_5").Cells.Item(i).Specific.value = oRecordSet.Fields.Item("U_CleaningType").Value
                        oMatrix.Columns.Item("V_6").Cells.Item(i).Specific.value = oRecordSet.Fields.Item("U_Location").Value
                        i = i + 1
                        oRecordSet.MoveNext()
                    Loop
                End If
            ...
            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed with SUCCESS", sFuncName)
            DuplicateJobSchedule = RTN_SUCCESS
        Catch ex As Exception
            sErrDesc = ex.Message
            Call WriteToLogFile(sErrDesc, sFuncName)
            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed with ERROR", sFuncName)
            DuplicateJobSchedule = RTN_ERROR
        End Try
    End Function

    Public Function JobSchedule_DeleteRow(ByRef oForm As SAPbouiCOM.Form, ByRef sErrDesc As String) As Long
        Dim oMatrix As SAPbouiCOM.Matrix

        Try
            sFuncName = "JobSchedule_DeleteRow"
            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Starting Function", sFuncName)
            ...
            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed with SUCCESS", sFuncName)
            JobSchedule_DeleteRow = RTN_SUCCESS
        Catch ex As Exception
            sErrDesc = ex.Message
            Call WriteToLogFile(sErrDesc, sFuncName)
            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed with ERROR", sFuncName)
            JobSchedule_DeleteRow = RTN_ERROR
        End Try
    End Function

    Private Sub ClearMatrix(ByVal oForm As SAPbouiCOM.Form)
        Dim oMatrix As SAPbouiCOM.Matrix
        oMatrix = oForm.Items.Item("matJobSch").Specific

        Dim iRowCount As Integer = oMatrix.VisualRowCount

        oMatrix.Columns.Item("V_2").Cells.Item(iRowCount).Specific.value = ""
        oMatrix.Columns.Item("V_3").Cells.Item(iRowCount).Specific.value = ""
        oMatrix.Columns.Item("V_4").Cells.Item(iRowCount).Specific.value = ""
        oMatrix.Columns.Item("V_5").Cells.Item(iRowCount).Specific.value = ""
        oMatrix.Columns.Item("V_6").Cells.Item(iRowCount).Specific.value = ""
    End Sub
