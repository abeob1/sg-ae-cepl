loadProject();
loadContentsTab();
$(document).ready(function() { // Signature	// initialize signature	

    $("#supSign,#clSign").jSignature({
        color: "#000000",
        height: '110px',
        width: '100%'
    });
            ...
            if ($(this).val() != "") {
				...
				postToServer(url, "POST", JSON.stringify(mProjectData), function(data) {
                        if (data != ""&& data.length > 0) {
                            ...
                            $("#scopeofwork").val(data["ScopeofWork"]); // Clear all values inside tab 
        ...
        (function() {
                validateRequired(function(errorMsg) {
					...
					$(".descoth").each(function(){
						...
						var contObj = {};
						contObj["Description"] = encodeURIComponent($(this).parents(":eq(1)").find
...
(".quest").text().trim());
						...
						contObj["Nonactive"] =  getCheckboxStatus($(this).parents(":eq(1)").find

(".Nonactive"));
						contObj["TFogging"] =  getCheckboxStatus($(this).parents(":eq(1)").find

(".TFogging"));
						contObj["TMisting"] =  getCheckboxStatus($(this).parents(":eq(1)").find

(".TMisting"));
						contObj["TResidual"] =  getCheckboxStatus($(this).parents(":eq(1)").find

(".TResidual"));
						contObj["TBaits"] =  getCheckboxStatus($(this).parents(":eq(1)").find(".TBaits"));
						contObj["TDusting"] =  getCheckboxStatus($(this).parents(":eq(1)").find
...
(".TDusting"));
						...
						contObj["Location"] = encodeURIComponent( $(this).parents(":eq(1)").find
...
(".Location").val());
    ...
    $("#attach").click(function() {
            var
                ...
                url = CONFIG.OPERATIONS_BASEPATH + "/Attachments";
            var companyUser = sessionStorage["user"];
            if
...
if(data[0] && data[0].Result=="Success"){				
                    ...
                    alert(data[0].DisplayMessage); sessionStorage["epsFile"] = JSON.stringify(data[0].Attachments);
...
function loadContentsTab() {
    var mProjectData = {};
    var url = CONFIG.OPERATIONS_BASEPATH +

        "/MGet_ServiceReport_GetTemplate"
    var companyUser = sessionStorage["user"];
    if (companyUser) {
        companyUser =
    ...
    postToServer(url, "POST", JSON.stringify(mProjectData), function(data) {
        if (data != "" && data.length > 0) {
            ...
            $(data).each(function(i, item) {
               ...
               var itemHtm = '<tr class="crow_'+i+'"><td>'+(i+1)+'</td><td class="quest">'+decodeURIComponent(item

["Question"])+'</td><td class="text-center"><input type="checkbox" name="include" class="Include" value="Y"/></td>'+
							'<td class="text-center"><input type="checkbox" name="active" class="Active" 

value="Y"/></td><td class="text-center"><input type="checkbox" name="nonactive" class="Nonactive" value="Y"/></td>'+
							'<td class="text-center"><input type="checkbox" name="f" 

class="TFogging"/></td><td class="text-center"><input type="checkbox" name="m" class="TMisting"/></td>'+
							'<td class="text-center"><input type="checkbox" name="r" 

class="TResidual"/></td><td class="text-center"><input type="checkbox" name="b" class="TBaits"/></td>'+
							'<td class="text-center"><input type="checkbox" name="d" 

class="TDusting"/></td><td class="text-center"><input type="checkbox" name="t" class="TTraps"/></td>'+
							'<td class="text-center"><input type="checkbox" name="o" 

class="TOthers"/></td><td class="text-center"><input type="checkbox" name="igs" class="IGS" /></td>'+
							'<td class="text-center"><input type="checkbox" name="ags" 

class="AGS"/></td><td><input type="text" class="Location"/></td><td><input type="text" class="descoth" value=""/></td></tr>';		
...
function loadProject() {
    ...
    var
        ...
        companyUser = sessionStorage["user"];
    if (companyUser) {
        companyUser = JSON.parse(companyUser);
    }
    mProjectData

        ["sCompany"] = companyUser["CompanyCode"];
    mProjectData["sCurrentUserName"] = companyUser["UserName"];
    mProjectData

        ["sUserRole"] = companyUser["RoleName"];
    showLoading();
    postToServer(url, "POST", JSON.stringify(mProjectData), function(data) {
